...
images = convert_from_path('./orignalPDF/orignal.pdf', poppler_path = r'C:\Program Files\poppler-24.08.0\Library\bin')
import PyPDF2
...
for i in range(len(images)):
    ...
    inverted_image.save(f"./invertedimages/inverted_image{i}.jpg") 
    inverted_img_path = f"./invertedimages/inverted_image{i}.jpg"
    with Image.open(inverted_img_path) as finalimg:
        pdf = img2pdf.convert(finalimg.filename)
        with open( f"./pdfs/pdf{i}.pdf" , "wb") as file:
            file.write(pdf)
...
for pdf in pdfList:
  print(pdf)
  # open each pdf
  with open(f"./pdfs/{pdf}.pdf", 'rb') as pdfFile:
    # merge each file
    pdfMerge.append(PyPDF2.PdfReader(pdfFile))

# write the merged pdf 
pdfMerge.write('./finalPDF/final.pdf')
